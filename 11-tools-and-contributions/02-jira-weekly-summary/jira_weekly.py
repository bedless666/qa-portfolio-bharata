...
class JiraWeeklySummary:
    ...
    def _split_into_chunks(self, text: str, max_length: int = 850) -> List[str]:
        """
        Smart split text into chunks under max_length
        - Detects section headers (e.g., **🛒 Buyer**, **📦 Seller-Fulfillment**)
        - Forces split BEFORE headers to avoid orphaned headers
        - Prevents splitting INSIDE member task blocks (keeps mention + tasks together)
        """
        ...
        for i, line in enumerate(lines):
            ...
            is_section_header = any(pattern in line for pattern in section_header_patterns)
            
            # Check if this line is a member mention (start of member block)
            is_member_mention = '**<mention-tag target=' in line
            
            # If we're about to exceed limit AND have content, try to split
            if current_length + line_length > max_length and current_chunk:
                # Only split if current line is a safe split point:
                # - Section header, OR
                # - Member mention (start of new member block)
                if is_section_header or is_member_mention:
                    chunks.append('\n\n'.join(current_chunk))
                    current_chunk = [line]
                    current_length = line_length
                else:
                    # Not a safe split point, include this line and force split after
                    current_chunk.append(line)
                    current_length += line_length
            # If current line is a section header and we have content, split before it
            elif is_section_header and current_chunk and current_length > 200:
                chunks.append('\n\n'.join(current_chunk))
                current_chunk = [line]
                current_length = line_length
            else:
    ...
    def build_message_single_team(self, jira_data: dict, team: str) -> dict:
        """Build message for single team (promotion/buyer/seller_fulfillment)"""
        logger.info(f"Building message for single team: {team}")
        ...
        member_sections = ""
        for email in team_config.get('members', []):
            tasks = tasks_by_assignee.get(email, [])
            member_sections += self._build_member_task_block(email, tasks)
        
        # Build stats section
        stats_section = f"""**📊 Stats:** {stats['total']} tickets | Testing: {stats['testing']} | Doing: {stats['doing']} | Done/Closed: {stats['done_closed']} | Waiting: {stats['waiting']}
...
<mention-tag target="seatalk://user?id=0"/>"""
        ...
        # Combine all - strip trailing newlines from member_sections to avoid excessive spacing
        all_text = member_sections.rstrip() + "\n\n" + stats_section
        chunks = self._split_into_chunks(all_text, max_length=850)
        
        # Get team emoji - map team names to emojis
        emoji_map = {
            'promotion': '🎯',
            'buyer': '🛒',
            'seller_fulfillment': '📦',
        ...
        team_emoji = emoji_map.get(team, '👥')
        team_name = team_config.get('team_name', team.replace('_', ' ').title())
        
        # Build message
        message = self._build_message_structure(
            chunks, 
            date_start, 
            date_end, 
            str(team_config['jira_filter']),
            title=f"{team_emoji} {team_name} - Weekly Summary"
        )
        
        # Save message
        with open(self.temp_dir / f"message_{team}.json", 'w') as f:
            json.dump(message, f, indent=2)
        
        logger.info(f"Single team message built: {len(chunks)} description elements")
...
<mention-tag target="seatalk://user?id=0"/>"""
        ...
        chunks_msg1 = self._split_into_chunks(all_text_msg1, max_length=850)
        ...
        chunks_msg2 = self._split_into_chunks(all_text_msg2, max_length=850)
    ...
    def _build_message_structure(self, chunks: List[str], date_start: str, date_end: str, filter_id: str, title: str = None) -> dict:
        """Build SeaTalk message structure"""
        # Use custom title if provided, otherwise default to All Teams
        title_text = title if title else f"👥 All Teams - Weekly Summary ({date_start} - {date_end})"
        
        elements = [
            {
                "element_type": "title",
                "title": {
                    "text": f"{title_text} ({date_start} - {date_end})"
                }
            }
        ]
    ...
    def run(self, team: str = "all_teams", mode: str = "test"):
        ...
        try:
            ...
            if team == "all_teams":
                ...
                # Single team (promotion, buyer, seller_fulfillment)
                message = self.build_message_single_team(jira_data, team)
                
                # Step 3: Send to webhook
                logger.info("📤 Step 3: Sending message to webhook...")
                webhook_id = self.config['teams'][team]['webhook_test'] if mode == 'test' else self.config['teams'][team]['webhook_production']
                
                success = self.send_to_webhook(message, webhook_id, 1)
